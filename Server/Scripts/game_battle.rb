...
module Game_Battler
  ...
  def make_damage_value(user, item, animation_id)
    ...
    execute_hp_damage(-value, false, animation_id)
  end
...
class Game_Client < EventMachine::Connection
  ...
  def attack_range
    ...
    if @target.type == Constants::TARGET_PLAYER && valid_target?(target) && $server.maps[@map_id].pvp && !target.admin? && !protection_level?(target)
...
module Game_Enemy
  ...
  def treasure
    if $server.clients[@target.id].in_party?
      $server.clients[@target.id].party_share(enemy.exp * EXP_BONUS, rand(enemy.gold).to_i, @enemy_id)
    else
  ...
  def disable
    ...
    if enemy.disable_switch_id >= Configs::MAX_PLAYER_SWITCHES
      $server.change_global_switch(enemy.disable_switch_id, !$server.switches[enemy.disable_switch_id - Configs::MAX_PLAYER_SWITCHES])
