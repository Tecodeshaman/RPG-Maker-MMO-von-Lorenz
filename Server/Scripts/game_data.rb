...
module Game_Data
  ...
  def load_game_data
	...
	def load_motd
		puts('Loading message of the day...')
		@motd = File.open('motd.txt', 'r:bom|UTF-8', &:read)
		#@motd = File.read('motd.txt')
	end

	def load_banlist
		puts('Loading list of banned accounts...')
	...
	def save_game_data
		puts('Saving all data...'.colorize(:green))
		save_motd
		save_banlist
