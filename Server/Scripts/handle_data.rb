...
# ** Handle_Data
#------------------------------------------------------------------------------
#  Este módulo recebe as mensagens do cliente.
#------------------------------------------------------------------------------
#  Autor: Valentine
#==============================================================================
...
module Handle_Data
	...
	def handle_messages_game(client, header, buffer)
		...
		when Constants::PACKET_CLOSE_BANK
			handle_close_bank(client)
		when Constants::PACKET_CLOSE_SHOP
			handle_close_shop(client)
	...
	def handle_login(client, buffer)
		...
		send_login(client)
		puts("#{user} logged in with IP #{client.ip}.")
	end
	...
	def handle_new_character(client, buffer)
		...
		return if class_id < 1 || class_id > Configs::MAX_CLASSES
	...
	def handle_remove_character(client, buffer)
		...
		return unless pass == client.pass
	...
	def handle_bank_item(client, buffer)
		...
		return if amount > 0 && client.full_bank?(container[item_id])
	...
	def handle_close_bank(client)
		client.close_bank
	end

	def handle_close_shop(client)
		client.close_shop
	end
